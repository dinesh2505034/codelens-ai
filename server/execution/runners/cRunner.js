...
import { instrumentC } from '../instrumentation/cInstrumenter.js';
...
export async function runRealC(code, customInputs = '', options = {}) {
  ...
  try {
    ...
    // 1. REAL COMPILATION OF ORIGINAL SOURCE VIA GCC / CLANG
    ...
    try {
      const instrumentedCode = instrumentC(code);
      const instSrcName = 'inst_main.c';
      ...
      const instCompRes = await runProcessWithLimits(compilerInfo.binPath, ['-O0', instSrcName, '-o', instBinName], {
        cwd: sandboxDir,
        env,
        timeoutMs: 10000
      });
